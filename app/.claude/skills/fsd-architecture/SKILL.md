...
pages/
  some-page/
    api/                # 공유 쿼리 (GET). mutation은 ui/ 컴포넌트에 co-locate
    model/              # 데이터 타입, 변환 함수, Context Provider 스토어
    ui/                 # Fetcher Container + Content + 하위 컴포넌트
    index.tsx           # 진입점 (export default)
```

- `index.tsx`가 진입점 — 별도 `{PageName}Page.tsx` + `index.ts` barrel 불필요
- `ui/fetch-{page}.tsx` = Fetcher Container (쿼리 실행, loading/error 분기)
- `ui/{page}-content.tsx` = Content (순수 UI, Apollo import 없음)
- `ui/{component}.tsx` = 하위 컴포넌트 (자체 mutation co-locate 가능)
